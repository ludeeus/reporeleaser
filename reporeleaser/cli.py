...
@click.option('--hide_sha', is_flag=True, help="Hide sha from description.")
@click.option('--version', '-V', is_flag=True, help='Print version.')
def cli(token, repo, release, test, hide_sha, version):
    ...
    if version:
        ...
        create_release = CreateRelease(token, repo, release, test, hide_sha)
